...
**Adversarial review of the whole build (2026-09-17) and the fixes that followed.**
Three independent reviewers (curate front end, Python side, built site) reported 43 findings after Christina hit three bugs in the curate page. The root cause of two of them was a CSS class-name collision (`.focal` on both the overlay grid and the thumbnail button). Notable changes made in response: the curate page validates a write strictly and rolls it back if the series would not load, so removing the last photograph of a published series is refused rather than breaking the build; the watcher snapshots before rebuilding so nothing written during a rebuild is lost; the landing slideshow now runs in site section order and `featured` order (it had been alphabetical section order and `images` order, matching the curate page only by coincidence); `dist/` is rebuilt from scratch on every build with hard-linked variants; a 1000px variant was added for phones and the 2400px one saved at quality 78; the page no longer depends on the script having run before images or the series head show; Inter is shipped once as a variable font; muted text was darkened to pass AA; every link has a 44px target; the sitemap dropped `lastmod`. The reviews' full reports stayed in the session; DECISIONS records only what changed.

## 2026-09-20

**Deployed to GitHub Pages from a public repository, chesun/photo-website.**
GitHub Pages on a free account requires a public repository, and the photographs are public on the site anyway. The workflow builds with the same script as the local build and caches processed variants between runs. The custom domain is configured on the GitHub side from day one; the DNS change at Squarespace is Christina's to make when she wants the switch, and until then visitors see the old site. No old URLs are redirected (decided 2026-09-12).
